...
//      data.flags = duplicate(this.item.data.flags);
  ...
  async updateWeight() {
    ...
    if (this.item.actor && (oldWeight !== totalWeight || itemWeight !== oldItemWeight)) {
      console.log(" doing item update in item collection")
  ...
  async _compactAll() {
    ...
    for (const itemData of items) {
      if (!itemData.flags.itemcollection) {
        ...
        } else if (mergedItems[itemData.name]) { // we would like to mrge but can't
