...
export class ItemSheet5eWithBags extends ItemSheet5e {
  ...
  async getData() {
    ...
    if (!["backpack", "loot"].includes(type)) {
...
//      data.flags = duplicate(this.object.data.flags);
    ...
    data.canImportExport = data.isOwned && !this.object.actor.isToken;
  ...
  async updateWeight() {
    ...
    if (this.object.actor) {
      ...
      this.object.data.flags.itemcollection.itemWeight = itemWeight;
      await this.object.actor.updateOwnedItem(this.object.data);
    } else {
      await this.object.update({"data.weight": totalWeight, "flags.itemcollection.itemWeight": itemWeight});
    }
  }
  ...
  async updateOwnedItem(itemChanges, options) {
    ...
    if (this.object.actor) {
      this.object.data.flags.itemcollection.contents = contents;
      await this.object.actor.updateOwnedItem(this.object.data);
    } else {
      await this.object.update({"flags.itemcollection.contents": contents});
    }
    this.render(false);
  }
  ...
  async deleteOwnedItem(deleted) {
    ...
    if (this.object.actor) {
      this.object.data.flags.itemcollection.contents = contents;
      await this.object.actor.updateOwnedItem(this.object.data);
    } else await this.object.update({"flags.itemcollection.contents": contents});
...
//      if (!this.canAdd(item)) return false;
    var items = duplicate(this.object.data.flags.itemcollection.contents);
    itemData.id = items.length ? Math.max(...items.map(i => i.id)) + 1 : 1;
    console.log(`ItemCollection | Created item ${itemData.id}`)
    delete itemData._id
    items.push(itemData);
    if (this.object.actor) {
      this.object.data.flags.itemcollection.contents = items;
      await this.object.actor.updateOwnedItem(this.object.data);
    } else {
      await this.object.update({"flags.itemcollection.contents": items});
    }
    await this.updateWeight();
    this.render(false);
    return true;
  ...
  canAdd(itemData) {
    ...
    if (this.object.data.data.capacity.type === "items") {
      let itemCount = 0 | this.object.data.flags.itemcollection.contents.reduce((val, itemData) => val + (itemData.data.quantity || 1), 0);
  ...
  async _itemExport(event) {
    ...
    if (this.object.actor && !items[idx].flags.itemcollection) { // not a bag
     ...
     // if (!this.object.actor.data._id) return;
      ...
      if (this.object.actor) {
        this.object.data.flags.itemcollection.contents = items;
        this.object.data.data.currency = bagCurrency;
        await this.object.actor.updateOwnedItem(this.object.data);
      } else await this.object.update({"flags.itemcollection.contents": items, "data.currency": bagCurrency});
...
//      await this.object.update({"data.currency": bagCurrency});
  ...
  async _itemSplit(event) {
    ...
    this.object.data.flags.itemcollection.contents = items;
    if (this.object.actor) {
      this.object.actor.updateOwnedItem(this.object.data);
    } else (this.object.update({"flags.itemcollection.contents": items}));
...
//    await this.deleteOwnedItem(id);
  ...
  async _compactAll() {
    ...
    if (this.object.actor) {
      this.object.data.flags.itemcollection.contents = items;
      await this.object.actor.updateOwnedItem(this.object.data);
    } else await this.object.update({"flags.itemcollection.contents": items});
    await this.updateWeight();
    this.render(false);
  }
  ...
  async _exportAll(event) {
    ...
    this.object.data.flags.itemcollection.contents = [];
    this.object.data.flags.itemcollection.goldValue = 0;
    await this.object.actor.update({"data.currency": currency});
    await this.object.actor.updateOwnedItem(this.object.data);
    await this.updateWeight();
    this.render(false);
  }
