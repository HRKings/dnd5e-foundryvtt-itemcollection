var ItemCollection = function () {
  ...
  class ItemSheet5eWithBags extends ItemSheet5e {
    ...
    async _updateWeight() {
      ...
      let newBagWeight = !equipped ? 0 : isFixed ? this.bag.data.data.weight.value : bagWeight;
    ...
    async _onDrop(event) {
      ...
      if ( data.data ) {
        ...
        if (this.canAdd(data.data)) {
            ...
            if (actor)  await actor.deleteOwnedItem(data.data.id);
            return;
        }
        // Item will not fit in the bag what to do?
        if (this.bag.actor) { // this bag is owned by an actor - drop into the inventory instead.
            let myActor = game.actors.get(this.bag.actor.data._id);
    ...
    async _importItemFromCollection(collection, entryId) {
      ...
      return pack.getEntity(entryId).then(ent => {
        ...
        if (this.canAdd(ent.data)) {
          ...
          console.log("getting actor to import")
          let actor = (this.bag.actor.data._id) ? game.actors.get(this.bag.actor.data._id) : undefined;
          if (actor) actor.createOwnedItem(ent.data, {});
        }
      });
    }

    async _importAllItemsFromActor() {
      if (!this.object.actor || this.object.actor.isToken) return;
...
//      this.object.actor.updateOwnedItem(changed, {});
    ...
    async _itemExport(event) {
      ...
      if (this.object.actor && this.object.actor.isToken) return; // does not work for tokens
    ...
    async _allItemsExport(event) {
      if (this.object.actor && !this.object.actor.isToken) { // if this is an owned item export all items into the owning actors inventory
