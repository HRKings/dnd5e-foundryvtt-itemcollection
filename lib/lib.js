...
//      this.render(true);
    ...
    async _deleteBagItem(deleted) {
      ...
      console.log(`itemcollection | Deleted Item ${deleted} from bag ${this.object.data._id}`);
    ...
    async _importAllItemsFromActor() {
      ...
      for (let itemData of allOwnedItems) {
        if ( (itemData.id !== this.object.data.id)   // don't import the bag itself
            ...
            && (itemData.type === "spell" ? this.object.getFlag("itemcollection", "importSpells.value") : !this.object.getFlag("itemcollection", "importSpells.value")) // isSpell XOR !importSpells
