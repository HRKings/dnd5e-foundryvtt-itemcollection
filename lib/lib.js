  ...
  class ItemSheet5eWithBags extends ItemSheet5e {
    ...
    async getData() {
      ...
      if (!item.data.flags.itemcollection) { // first time the sheet has been opened add itemcollection data to the item
        ...
        await item.setFlag("itemcollection", "fixedWeight", {"type": "Boolean", "label": "Fixed Weight", "value": false, "weight": 0});
      ...
      if (!item.getFlag("itemcollection", "fixedWeight.weight")) {
        item.setFlag("itemcollection", "fixedWeight.weight", item.getFlag("itemcollection", "fixedWeight.value") ? this.object.data.data.weight.value  : 0);
      }
    ...
    async _updateWeight() {
      ...
      this.object.actor.updateOwnedItem(duplicate(this.object.data));
    }
    ...
    _calcBagWeight () {
      ...
      if ( game.settings.get("dnd5e", "currencyWeight") ) {
        ...
        totalWeight = Math.round((totalWeight + numCoins / 5)) / 10;
      }
      return totalWeight;
    }
...
//      if (!this.canAdd(item)) return false;
    ...
    async _importAllItemsFromActor() {
      ...
      const items = this.object.getFlag("itemcollection", "contents");
      let allOwnedItems = duplicate(this.object.actor.items);
      let count = 0;
      ...
      for (let item of allOwnedItems) {
        if ( (item.id !== this.object.data.id)   // don't import the bag itself
            ...
            && (capacity == 0 || (capacity > weight + this._calcItemWeight(item)) ) /* bag not too full */ ) 
        {
          await this.object.actor.deleteOwnedItem(item.id);
          items.push(item); 
          count += 1;
          weight += this._calcItemWeight(item);
        }
      }
      await this.object.setFlag("itemcollection", "contents", duplicate(items));
    ...
    async _exportAll(event) {
      ...
      ["pp", "gp", "sp", "cp"].forEach((denom) => currency[denom].value = 0);
