...
/*
      if (this.object.isOwned) {
        let actor = this.object.actor; // game.actors.get(this.object.actor.data._id);
...
//      if (!this.canAdd(item)) return false;
    ...
    async _importAllItemsFromActor() {
      ...
      this.object.setFlag("itemcollection", "contents", duplicate(items));
      this._updateWeight();
    ...
    async _itemExport(event) {
      ...
      if (this.object.actor && !items[idx].flags.itemcollection) { // not a bag
        ...
        this.object.setFlag("itemcollection", "contents", duplicate(items));
        this.object.setFlag("itemcollection", "currency", bagCurrency);
        this._updateWeight();
      }
    }
