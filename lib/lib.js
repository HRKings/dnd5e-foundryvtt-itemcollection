...
//      data.flags = duplicate(this.object.data.flags);
    ...
    for (let i of data.flags.itemcollection.contents){
      i.canEdit = !(i.flags.itemcollection);
      // i.selected = false;
...
//      await this.object.update({"data.currency": bagCurrency});
  ...
  update(data,options) {
    if (data.items) {
      this.object.setFlag("itemcollection", "contents", duplicate(data.items))
    }
    else this.object.update(data, options)
  }
